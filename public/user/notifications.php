...
/**
 * Link trong DB thường là đường dẫn từ thư mục public/ (vd: user/my-bookings.php, owner/bookings.php).
 * Trang này nằm trong public/user/ nên cần chỉnh href cho đúng.
 */
function user_notifications_resolve_href(?string $link): string
{
    if ($link === null || $link === '') {
        return '#';
    }
    ...
    if (str_starts_with($link, 'user/')) {
        return substr($link, strlen('user/'));
    }
    if (str_starts_with($link, 'owner/') || str_starts_with($link, 'admin/')) {
        return '../' . $link;
    }
    return '../' . ltrim($link, '/');
}
                            ...
                            <article class="notification-card <?php echo $isUnread ? 'unread' : ''; ?>">
                                <div>
                                    <div class="d-flex flex-wrap gap-2 align-items-center">
                                        <div class="notification-title"><?php echo htmlspecialchars((string)$notification['title'], ENT_QUOTES, 'UTF-8'); ?></div>
                                        <?php if ($isUnread): ?><span class="badge text-bg-primary">Mới</span><?php endif; ?>
                                    </div>
                                    <?php if (!empty($notification['body'])): ?>
                                        <div class="notification-body"><?php echo nl2br(htmlspecialchars((string)$notification['body'], ENT_QUOTES, 'UTF-8')); ?></div>
                                    <?php endif; ?>
                                    <div class="notification-meta">
                                        <i class="fas fa-clock"></i> <?php echo date('d/m/Y H:i', strtotime((string)$notification['created_at'])); ?>
                                        <span class="ms-2"><?php echo htmlspecialchars((string)$notification['type'], ENT_QUOTES, 'UTF-8'); ?></span>
                                    </div>
                                </div>
                                <div class="d-flex flex-wrap gap-2 justify-content-end align-content-start">
                                    <?php if (!empty($notification['link'])): ?>
                                        <a class="btn btn-outline-primary btn-sm" href="<?php echo htmlspecialchars(user_notifications_resolve_href($notification['link']), ENT_QUOTES, 'UTF-8'); ?>">Mở</a>
                                    <?php endif; ?>
                                    <?php if ($isUnread): ?>
                                        <form method="POST">
