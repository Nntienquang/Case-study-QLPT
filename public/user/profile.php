...
$stmt = $db->prepare("SELECT id, name, email, phone, avatar, password, created_at FROM users WHERE id = ?");
    ...
    if ($action === 'update_profile') {
        ...
        if (empty($name)) {
            ...
            if ($message_type !== 'danger') {
                ...
                $stmt->bind_param("sssi", $name, $phone, $avatar_path, $user_id);
...
$display_avatar = !empty($user['avatar']) ? '../' . $user['avatar'] : 'https://ui-avatars.com/api/?name=' . urlencode($user['name'] ?? 'User') . '&background=667eea&color=fff';
?>
<!DOCTYPE html>
<html lang="vi">
<head>
                ...
                <div class="row">
                    <div class="col-lg-8">
                        <div class="profile-card">
                            <h5 class="card-header-title"><i class="fas fa-user-edit me-2 text-primary"></i> Chỉnh sửa thông tin & Ảnh đại diện</h5>
                            <form method="POST" enctype="multipart/form-data">
                                <input type="hidden" name="action" value="update_profile">

                                <div class="avatar-preview-wrap">
                                    <img src="<?php echo $display_avatar; ?>" alt="Avatar" class="avatar-preview" id="preview-img">
                                    <div>
                                        <label class="form-label d-block">Ảnh đại diện mới</label>
                                        <input type="file" name="avatar" class="form-control form-control-sm" accept="image/png, image/jpeg, image/gif" onchange="previewAvatar(this)">
                                        <small class="text-muted mt-1 d-block">Định dạng: JPG, PNG, GIF. Tối đa 2MB.</small>
                                    </div>
                                </div>
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="tel" name="phone" class="form-control" value="<?php echo htmlspecialchars($user['phone'] ?? ''); ?>" placeholder="Ví dụ: 0912345678">
                                </div>
                        ...
                        <div class="profile-card">
                            <h5 class="card-header-title"><i class="fas fa-lock me-2 text-primary"></i> Đổi mật khẩu</h5>
                            <form method="POST">
                    ...
                    <div class="col-lg-4">
                        <div class="profile-card">
                            ...
                            <div class="info-item">
                                ...
                                <div class="info-value">
                                    <span class="badge bg-success bg-opacity-10 text-success border border-success border-opacity-25 px-2 py-1">Đang hoạt động</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="profile-card">
                            ...
                            <a href="#" class="action-link" onclick="alert('Tính năng dành cho chủ phòng. Vui lòng nâng cấp tài khoản!'); return false;">
                                <i class="fas fa-list-alt"></i> Quản lý tin đăng
                            </a>
                            <a href="#" class="action-link" onclick="alert('Chức năng gửi báo cáo đang được phát triển!'); return false;">
